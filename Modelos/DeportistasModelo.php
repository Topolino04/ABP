...
function conexionBD()
	...
	function devolverfoto($user){
			...
			$foto="";
			$mysqli=$this->conexionBD();
			$query="SELECT * FROM Deportista where Usuario='$user'";
			$resultado=$mysqli->query($query);
		if(mysqli_num_rows($resultado)){
		...
		function comprobardnideportista($dni){
			
		$mysqli=$this->conexionBD();
		$query="SELECT * FROM `Deportista` WHERE `DNI`='$dni' ";
		$resultado=$mysqli->query($query);
		...
		function creararrayMonitores(){

			$this->conexionBD();
		$file = fopen("../Archivos/Arraymonitores.php", "w");

		fwrite($file,"<?php class consultar1234 { function array_consultar(){". PHP_EOL);
		...
		if(mysqli_num_rows($resultado)){
			...
			foreach($filas as $fila)
			{
				...
				fwrite($file,"array(\"nombre\"=>'$nombre',\"dni\"=>'$dni')," . PHP_EOL);
...
function altaDeportista($nombre,$dni,$fecha,$email,$apell1,$foto,$usuario,$telefono,$pass,$tipo)
{
	$mysqli=$this->conexionBD();
	$boolean=true;
	...
	//$resultado=$mysqli->query("INSERT INTO `deportista`( `Tipo`, `Nombre`, `Apellido_1`, `Apellidoss`, `DNI`, `Telefono`, `Usuario`, `Password`, `emailgit`, `FechaNac`) VALUES ('$tipo','$nombre','$apell','$apell1','$dni','$telefono','$usuario','$pass','$telefono','$fecha')");
		if($mysqli->query("INSERT INTO `Deportista`(`Usuario`, `Password`, `Nombre`, `Apellidos`, `FOTO`,`DNI`, `Email`, `FechaNac`, `Telefono`, `Tipo`) VALUES ('$usuario','$pass','$nombre','$apell1','$foto','$dni','$email','$fecha','$telefono','$tipo')")==TRUE)
		{
 ...
 function modificarDeportista($nombre,$dni,$fecha,$email,$apell1,$foto,$usuario,$telefono,$pass,$tipo){

 	$mysqli=$this->conexionBD();
 	$query="UPDATE `Deportista` SET `Usuario`='$usuario',`Password`='$pass',`Nombre`='$nombre',`Apellidos`='$apell1',`FOTO`='$foto',`DNI`='$dni',`Email`='$email',`FechaNac`='$fecha',`Telefono`='$telefono',`Tipo`='$tipo' WHERE DNI='$dni'";
 	if($mysqli->query($query)==TRUE){
		?>
		<script>
